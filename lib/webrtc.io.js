...
(function () {
  ...
  function checkDataChannelSupport() {
    try {
  ...
  rtc.checkDataChannelSupport = checkDataChannelSupport
  //  检测数据通信可行性 
  ...
  rtc.sendOffer = async function (socketId) {
    ...
    //  兼容 谷歌56+
    var session_description = await pc.createOffer(sdpConstraints.mandatory)
    session_description.sdp = preferOpus(session_description.sdp);

    await pc.setLocalDescription(session_description)
    rtc._socket.send(JSON.stringify({
  ...
  rtc.receiveOffer = function (socketId, sdp) {
    var pc = rtc.peerConnections[socketId];
    confirm("确认连接?" + socketId) && rtc.sendAnswer(socketId, sdp);
  };

  rtc.sendAnswer = async function (socketId, sdp) {
    var pc = rtc.peerConnections[socketId];
    if (sdp) {
      await pc.setRemoteDescription(new nativeRTCSessionDescription(sdp));
    }
    var session_description = await pc.createAnswer(sdpConstraints.mandatory)
    await pc.setLocalDescription(session_description);
  ...
  rtc.attachStream = function (stream, element) {
    ...
    if (navigator.mozGetUserMedia) {
      ...
      element.src = URL.createObjectURL(new MediaSource(stream));
    }
  };
